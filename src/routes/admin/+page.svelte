...
<div class="container mt-4">
        ...
        <ul class="list-group">
            {#each Object.entries(data.users) as [username, user]}
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <button class="btn btn-sm btn-primary" on:click={() => changeStatus(username, 0)}>Set User</button>
                        <button class="btn btn-sm btn-warning" on:click={() => changeStatus(username, 1)}>Set Admin</button>
                    </div>
                </li>
            {/each}
        </ul>
    {/if}
    {#if !data.logged_in}
        <div class="alert alert-danger" role="alert">You are not logged in. Please log in to access the admin panel.</div>
    {/if}
</div>