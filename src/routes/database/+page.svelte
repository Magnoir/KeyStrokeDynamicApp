<script lang="ts">
	...
	onMount(async () => {
		...
		if (result.data && typeof result.data === "object") {
			const records: Record[] = Object.entries(result.data).flatMap(([key, record]) =>
	...
	async function deleteRecord(id: string, username: string | undefined, index: number) {
		try {
								...
								<div class="card-header d-flex justify-content-between align-items-center">
									...
									<div>
										...
										<button
											...
											on:click={() => deleteRecord(record.id, record.username, index)}>
											Delete
										</button>
									</div>
								</div>
